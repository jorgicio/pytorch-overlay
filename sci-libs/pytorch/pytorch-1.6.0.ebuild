...
PATCHES=(
	"${FILESDIR}"/1.6/${PN}-1.6.0-setup.patch
	"${FILESDIR}"/1.6/${PN}-1.6.0-skip-tests.patch
	"${FILESDIR}"/1.6/${PN}-1.6.0-global-dlopen.patch
...
src_prepare() {
	...
	if use cuda; then
		cd ../nccl || die
		eapply "${FILESDIR}"/1.6/${PN}-1.6.0-nccl-nvccflags.patch
		ln -s . nccl || die
	...
	cd ../tensorpipe || die
	eapply "${FILESDIR}"/1.6/${PN}-1.6.0-tensorpipe-unbundle-libuv.patch
}
